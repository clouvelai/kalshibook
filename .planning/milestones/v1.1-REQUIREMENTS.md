# Requirements Archive: v1.1 Python SDK
...
- [x] **SDKC-02**: User can initialize client with API key (`KalshiBook(api_key="...")`)
- [x] **SDKC-03**: User can initialize client from environment variable (`KalshiBook.from_env()`)
- [x] **SDKC-04**: Client supports both sync and async usage patterns
- [x] **SDKC-05**: SDK raises typed exceptions matching API error codes (AuthenticationError, RateLimitError, CreditsExhaustedError, etc.)
- [x] **SDKC-06**: All responses include credit usage metadata (credits_used, credits_remaining)

### Data Endpoints

- [x] **DATA-01**: User can query reconstructed orderbook at any timestamp (`client.get_orderbook(ticker, timestamp)`)
- [x] **DATA-02**: User can iterate raw deltas with auto-pagination (`for delta in client.list_deltas(ticker, start, end)`)
- [x] **DATA-03**: User can iterate trades with auto-pagination (`for trade in client.list_trades(ticker, start, end)`)
- [x] **DATA-04**: User can list available markets with coverage dates (`client.list_markets()`)
- [x] **DATA-05**: User can get market details (`client.get_market(ticker)`)
- [x] **DATA-06**: User can query candles (`client.get_candles(ticker, interval)`)
- [x] **DATA-07**: User can query settlements (`client.list_settlements()`)
- [x] **DATA-08**: User can query events and event hierarchy (`client.list_events()`, `client.get_event(ticker)`)

### DataFrame Support

- [x] **DFRA-01**: All list/paginated responses support `.to_df()` conversion to pandas DataFrame
- [x] **DFRA-02**: pandas is an optional dependency (`pip install kalshibook[pandas]`)

### Documentation

- [x] **DOCS-01**: mkdocs-material docs site with Getting Started guide
- [x] **DOCS-02**: Authentication guide covering API key setup and from_env()
- [x] **DOCS-03**: API reference auto-generated from docstrings
- [x] **DOCS-04**: Code examples for each endpoint

### Packaging

- [x] **PACK-01**: SDK published to PyPI with py.typed marker
...
| SDKC-02 | Phase 9 | Complete |
| SDKC-03 | Phase 9 | Complete |
| SDKC-04 | Phase 9 | Complete |
| SDKC-05 | Phase 9 | Complete |
| SDKC-06 | Phase 9 | Complete |
...
| DFRA-01 | Phase 11 | Complete |
| DFRA-02 | Phase 11 | Complete |
