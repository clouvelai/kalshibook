"""Trade history query endpoint -- POST /trades.

Returns paginated trade execution records for a market within a time range,
...
from src.api.models import TradeRecord, TradesRequest, TradesResponse
...
async def get_trades(
    ...
    """Query trade execution history for a market within a time range.
    ...
    async with pool.acquire() as conn:
        if body.cursor is not None:
            ...
            rows = await conn.fetch(
                """
                SELECT id, trade_id, market_ticker, yes_price, no_price,
                       count, taker_side, ts
                FROM trades
