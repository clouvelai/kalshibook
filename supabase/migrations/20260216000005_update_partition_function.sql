-- Extend partition management to create daily trades partitions alongside deltas
...
BEGIN
    ...
    -- Create daily trades partitions
    FOR i IN 0..days_ahead LOOP
        partition_date := CURRENT_DATE + i;
        partition_name := 'trades_' || to_char(partition_date, 'YYYY_MM_DD');
        start_date := to_char(partition_date, 'YYYY-MM-DD');
        end_date := to_char(partition_date + 1, 'YYYY-MM-DD');
